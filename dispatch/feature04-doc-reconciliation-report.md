...
|---------|----------------|-----------------|---------|
| `candle_id` (as field on SignalGenerated or Signal) | ✅ Not present (only `_seen_candle_ids` set for dedup + explicit "no candle_id" statement) | Present in feature 08 (unrelated) | **Clean** |
...
- ✅ Engine validates candle instrument, timeframe, completeness; rejects duplicates
...
All required changes from `dispatch/feature04-doc-reconciliation.md` have been applied:
...
6. ✅ Documentation aligned with single-user, paper-first, broker-agnostic, Decimal/UUID, fail-closed goals
