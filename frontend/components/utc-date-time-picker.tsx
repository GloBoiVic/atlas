      ...
      <div className="flex flex-wrap gap-2">
        <Popover>
          <PopoverTrigger
            ...
            aria-label={`${label} date${valid ? `, ${formatUtcWallClock(`${value}:00Z`)}` : ''}`}
            aria-expanded={open}
            disabled={disabled}
            className="form-control inline-flex min-h-10 items-center gap-2 text-left font-normal"
          >
            <CalendarDays
              className="size-4 text-atlas-foreground-muted"
              aria-hidden
            />
