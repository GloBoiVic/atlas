...
const DisplayTimeZoneContext = createContext<{
  timeZone: DisplayTimeZone;
  setTimeZone: (zone: DisplayTimeZone) => void;
}>({ timeZone: DEFAULT_DISPLAY_TIME_ZONE, setTimeZone: () => undefined });
...
export function Providers({ children }: { children: React.ReactNode }) {
  ...
  const setTimeZone = (zone: DisplayTimeZone) => {
    ...
    try {
      window.localStorage.setItem(DISPLAY_TIME_ZONE_STORAGE_KEY, zone);
    } catch {
      /* safe fallback */
    }
  };
