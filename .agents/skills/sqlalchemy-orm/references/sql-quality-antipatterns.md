...
for author in authors:
    ...
    books = session.scalars(select(Book).where(Book.author_id == author.id)).all()
...
session.execute(update(User).where(User.inactive.is_(True)).values(archived=True))
```
...
**Compliant:**
```python
session.execute(text("SELECT id FROM users WHERE email = :email"), {"email": email})
